...
-(id)initWithFrame:(CGRect)frame andCellWidth:(CGFloat )width andviews:(NSArray *)views_{
    ...
    if (self != nil) {
        ...
        CGRect frame1 = CGRectMake(0, pageControl_.frame.size.height, frame.size.width, frame.size.height);
...
- (void) scrollViewWillBeginDragging:(UITableView *)tableView {
    ...
    currOffsetY = tableView.contentOffset.y;
}
...
-(void) scrollViewWillEndDragging:(UIScrollView*)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint*)targetContentOffset {
    ...
    }else if (fabs(velocity.y)> 1.5f && fabs(velocity.y) < 3.0f  ){
...
-(void)setCurrentPageForPageControl{
    ...
    UIView *v = [array_ objectAtIndex:1];
    v_tag = v.tag;
